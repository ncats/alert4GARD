...
def get_GARD_diseases_list():
  ...
  with GraphDatabase.driver(myConfig['diseases']['url']) as driver:
    try:
...
def save_articles_pubmed(mindate, maxdate):
  ...
  with GraphDatabase.driver(myConfig['alerts']['url'],auth=(myConfig['alerts']['username'], myConfig['alerts']['password'])) as driver:
    try:
      with driver.session() as session:
...
def capture_new_article_annotations(date_since):
  with GraphDatabase.driver(myConfig['alerts']['url'],auth=(myConfig['alerts']['username'], myConfig['alerts']['password'])) as driver:
    try:
      with driver.session() as session:
        res = session.run("MATCH (n:Article) WHERE date(datetime(n.created)) > date({date_since}) RETURN n.pubmed_id", parameters={"pmid":pubmedID})
...
def save_initial_articles():
  ...
  initial_articles_list = capture_new_article_annotations(mindate)
  with open('initial_articles.txt', 'w') as filehandle:
    for x in initial_articles_list:
        filehandle.write('%s\n' % x)
...
def save_new_articles():
  ...
  initial_articles_list = capture_new_article_annotations(yesterday)
  with open(f"initial_articles{yesterday}.txt", 'w') as filehandle:
    for x in initial_articles_list:
        filehandle.write('%s\n' % x)
